...
CLI 参数：`--full`（全量）、`--incremental`（增量，默认）、`--rate <秒>`（可选，腾讯接口请求间隔）

**输入**：`codes.txt`、`stock_api_specs.json`、`data/date/raw/{code}.csv`（增量模式）

**处理**：
- 读取股票代码列表，根据代码开头生成市场前缀（`sh`/`sz`/`bj`）
- 请求腾讯财经前复权日 K 接口（默认限速 1 次/秒，最多重试 3 次，可通过 `--rate` 调整）
- 前复权数据为空时回退到不复权日 K
...
**全量模式**：从头采集所有股票的全部日 K 数据（默认 1024 条/只，覆盖约 4 个交易年度）

**`--rate` 限频实测**：
- 默认间隔 0.5 秒/次（可通过 `--rate` 参数覆盖）
- 接口返回 429/503 时自动指数退避（2s → 4s → 8s）并降速恢复
- 建议逐步测试：`--rate 0.8` → `--rate 0.6` → `--rate 0.5`，观察日志中是否出现限频标记
...
- 前复权为空：自动回退不复权
- HTTP 429/503（限频）：自动指数退避（2^attempt 秒），重试 3 次后仍失败则跳过
...
| 2026-07-20 | v1.0 | KLINE_LIMIT 320→1024，覆盖 2023-01-01 以来的日K数据 |
| 2026-07-20 | v1.0 | 新增 `--rate` CLI 参数，支持实测腾讯接口安全限频上限 |
| 2026-07-20 | v1.0 | http_get/http_get_json 增加 429/503 检测与指数退避机制 |
| 2026-07-20 | v1.0 | TENCENT_RATE_LIMIT 统一调整为 0.5 秒/次 |
| 2026-07-20 | v1.0 | KLINE_LIMIT_FRONTEND 260→1024，K线图展示全部历史数据 |